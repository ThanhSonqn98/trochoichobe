  ...
  return (
    <div className="flex flex-col items-center min-h-full max-w-2xl mx-auto pt-4 px-4">
      ...
      <div className="bg-white rounded-3xl shadow-xl p-8 w-full border-b-8 border-lime-200 flex flex-col items-center relative overflow-hidden">
        {feedback === 'correct' && <><div className="absolute inset-0 bg-green-100/95 flex items-center justify-center z-20 flex-col animate-bounce-in"><Check size={80} className="text-green-500 mb-2" /><span className="text-3xl font-bold text-green-600">Đúng rồi!</span></div><Fireworks /></>}
        {feedback === 'wrong' && <div className="absolute inset-0 bg-red-100/95 flex items-center justify-center z-20 flex-col animate-shake"><span className="text-3xl font-bold text-red-600">Thử lại nhé!</span></div>}
        <h2 className="text-2xl font-bold text-slate-700 mb-8 text-center">Bé hãy tìm: <br/><span className="text-4xl text-lime-600 block mt-2">{level.target.name}</span></h2>
        <div className="grid grid-cols-2 gap-6 w-full max-w-xs">{level.options.map((opt, idx) => { const Icon = opt.icon; return (<button key={idx} onClick={() => handleAnswer(opt)} className="bg-slate-50 border-4 border-slate-100 hover:border-lime-300 rounded-3xl p-6 flex items-center justify-center aspect-square shadow-md transition-all active:scale-95 group"><Icon size={70} className={`${opt.color} group-hover:scale-110 transition-transform`} strokeWidth={2.5} /></button>)})}</div>
      </div>
    </div>
  );
};
...
const App = () => {
  ...
  const addScore = (points) => { setTotalScore(prev => prev + points); };
  return (
    <div className="min-h-screen bg-[#FDF6E3] font-sans selection:bg-purple-200">
      ...
      <style>{`@keyframes bounce-in {0%{transform:scale(0.3);opacity:0}50%{transform:scale(1.05)}70%{transform:scale(0.9)}100%{transform:scale(1);opacity:1}}@keyframes shake{0%,100%{transform:translateX(0)}25%{transform:translateX(-10px)}75%{transform:translateX(10px)}}@keyframes pulse-slow{0%,100%{transform:scale(1)}50%{transform:scale(1.1)}}.animate-bounce-in{animation:bounce-in 0.5s ease-out forwards}.animate-shake{animation:shake 0.3s ease-in-out}.animate-pulse-slow{animation:pulse-slow 2s infinite ease-in-out}.rotate-y-180{transform:rotateY(180deg)}`}</style>
    </div>
  );
};
